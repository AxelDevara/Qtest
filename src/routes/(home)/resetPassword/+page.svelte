    ...
    if (password == confirm) {
      if (password.length < 8) {
        ...
        try {
          ...
          toastSuccess("Check your email for the login link!");
          loading = false;
          await goto("/");
        } catch (error) {
      ...
      toastFailed("Invalid password confirmation");
    }
  };
    ...
    <div class="mt-6">
      <div>
        ...
        <input
          ...
          id=""
          placeholder="Your Password "
          class="w-full px-4 py-2 mt-1 text-base text-black transition duration-500 ease-in-out transform border-transparent bg-neutral-700 focus:border-gray-500 focus:bg-white focus:outline-none focus:shadow-outline focus:ring-2 ring-offset-current ring-offset-2 "
        />
      </div>
      ...
      {#if !loading}
        <button
          ...
          class="block rounded-md w-full px-4 py-3 mt-6 font-semibold text-white transition duration-500 ease-in-out transform bg-blue-500 hover:bg-black focus:shadow-outline focus:outline-none focus:ring-2 ring-offset-current ring-offset-2 "
