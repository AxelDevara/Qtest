<script>
  ...
  export let inputbg = 'bg-neutral-100';
  export let inputText = 'text-black';
...
<div class={`flex flex-col md:text-base text-sm ${$$props.class}`}>
  ...
  <div class={'relative'}>
    <input
      in:fade|local={{ duration: 500 }}
      on:change
      on:focus={(e) => (focus = true)}
      on:blur={(e) => (focus = false)}
