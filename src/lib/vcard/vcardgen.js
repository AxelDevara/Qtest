...
export const genvcard = async (prop, team, dateConnected, description) => {
  ...
  if (dateConnected) {
    vCard.note =
      ...
      new Date(new Date(dateConnected).setUTCHours(7)).toDateString().slice(4) + (description != null  ? '\n' + description : '');
  } else {
    vCard.note =
      'Added via Qubic card on ' +
      new Date(new Date().setUTCHours(7)).toDateString().slice(4) + (description != null  ? '\n' + description : '');
  }
